...
class PandasBatchProcessor(BatchProcessor):
    ...
    def __init__(
        ...
        super().__init__(DataEngine.PANDAS, batch_size, max_workers, show_progress, settings)
        try:
            import pandas as pd
    ...
    def iter_batches(
        self, data: "pd.DataFrame", batch_size: Optional[int] = None
...
class SparkBatchProcessor(BatchProcessor):
    ...
    def __init__(
        ...
        super().__init__(DataEngine.SPARK, batch_size, max_workers, show_progress, settings)
        try:
    ...
    def iter_batches(
        self, data: "DataFrame", batch_size: Optional[int] = None
        ...
        for offset in range(0, total_rows, batch_size):
            # Use offset/limit to get batches
            # Note: This requires full scan each time - not efficient for large data
...
class BatchProcessorFactory:
    ...
    def create(
        ...
        return processor_class(batch_size, max_workers, show_progress, settings)
    ...
    def create_auto(
        ...
        if isinstance(data, list | tuple | Generator):
            ...
            return ListBatchProcessor(batch_size, max_workers, show_progress, settings)

        engine = cls._detect_engine(data)
        return cls.create(engine, batch_size, max_workers, show_progress, settings)
